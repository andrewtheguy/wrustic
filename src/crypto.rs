...
/// key (typically the WebAuthn PRF output). Output is single-line:
...
/// boot from either an age identity (default) or a passkey PRF output
/// (`--experimental-passkey`), then threaded into `config::load` / `save`.
pub enum Cipher {
    Age {
        identity: Identity,
        recipient: Recipient,
